% ber_modulasi_dpsk.m
% Menghitung BER dari modulasi DPSK melalui GNU Octave
% ramhdi 13/04/2020
...
fc = 2e2; % carrier
Rb = 1e2; % bitrate
oversamp=4; % minimum oversamp = 4fc/Rb
N = 100;
...
for iter_snr = 1:1:length(SNR)
  ...
  ber_theo(iter_snr) = 0.5*exp(-10.^(SNR(iter_snr)/10));
  ...
  for iter_n = 1:1:n_iter
    ...
    [xt,bit_expand,bit_encoded,n] = dpskmod(bit_in, fc, Rb, oversamp);
    noise =  sqrt(fsamp*0.5*10^(-SNR(iter_snr)/10))*randn(1,length(xt));
    xr = xt + noise; %xr = awgn(xt, SNR(iter_snr),'measured');
    %[bit_out, ich, qch, iiad, qiad, iad] = dpskdemod2(xr, fc, Rb, oversamp);
    [bit_out, mix, iad] = dpskdemod(xr, Rb, oversamp);
