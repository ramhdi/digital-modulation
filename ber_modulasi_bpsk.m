% ber_modulasi_bpsk.m
% Menghitung BER dari modulasi BPSK melalui GNU Octave
% ramhdi 08/04/2020
...
fc = 2e2; % carrier
Rb = 1e2; % bitrate
oversamp=10; % minimum oversamp = 4fc/Rb
N = 100;
...
for iter_snr = 1:1:length(SNR)
  ...
  ber_theo(iter_snr) = qfunc(sqrt(2*10^(SNR(iter_snr)/10)));
  ...
  for iter_n = 1:1:n_iter
    ...
    [xt,bit_expand,n] = bpskmod(bit_in, fc, Rb, oversamp);
